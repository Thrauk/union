...
class RemoteSwitchButtonWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<AddOpenRoleBloc, AddOpenRoleState>(
      buildWhen: (AddOpenRoleState previous, AddOpenRoleState current) => previous.isRemotePossible != current.isRemotePossible,
      builder: (BuildContext context, AddOpenRoleState state) {
        return Padding(
          padding: const EdgeInsets.symmetric(vertical: 4.0),
          child: Row(
            children: [
              const Text('Is remote work possible', style: AppStyles.textStyleBodyBig),
              const SizedBox(width: 6),
              SimpleSwitch(
                value: state.isRemotePossible,
                onChange: (bool switchValue) {
                  context.read<AddOpenRoleBloc>().add(IsRemotePossibleChanged(switchValue));
                },
              ),
            ],
          ),
