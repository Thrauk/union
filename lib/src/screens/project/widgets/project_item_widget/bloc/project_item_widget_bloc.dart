...
class ProjectItemWidgetBloc extends Bloc<ProjectItemWidgetEvent, ProjectItemWidgetState> {
  ProjectItemWidgetBloc(this._projectRepository, this._projectLikesRepository) : super(const ProjectItemWidgetState()) {
  ...
  final FirebaseProjectLikesRepository _projectLikesRepository;
  ...
  Future<void> _getDetails(GetDetails event, Emitter<ProjectItemWidgetState> emit) async {
    ...
    final bool isLiked = await _projectLikesRepository.verifyIfUserLiked(event.projectId, event.loggedUid);
  ...
  Future<void> _likePressed(LikePressed event, Emitter<ProjectItemWidgetState> emit) async {
    if (state.isLiked == true) {
      _projectLikesRepository.removeLikeFromProject(event.project.id, event.uid);
      emit(state.copyWith(isLiked: false, likesNr: state.likesNr - 1));
    } else {
      _projectLikesRepository.addLikeToProject(event.project.id, event.uid);
  ...
  Future<void> _getLikesNr(GetLikesNr event, Emitter<ProjectItemWidgetState> emit) async {
    final int likes = await _projectLikesRepository.getLikesNumber(event.projectId);
